...
+function($) {
    ...
    var Tooltip = function(element, options) {
		...
		this.namespace = 'tooltip-'+ Math.round(Math.random()*100000);
	...
	Tooltip.prototype = {
		...
		_init: function() {
			...
			if (document.querySelector) {
				...
				if (self.$el.data('tooltip-initialTitle') === undefined) {
					...
					self.$el.data('tooltip-initialTitle', initialTitle);
				}
			...
			self.options.functionBefore.call(self.$el, self.$el, function() {
				...
				if (self.enabled && self.Content !== null) {
					...
					if (self.Status !== 'hidden') {
						...
						if (self.Status === 'disappearing') {
							...
							if (GRID.browserSupportsTransitions()) {
								...
								self.$tooltip
									.clearQueue()
									.removeClass('tooltip-dying')
									.addClass('tooltip-'+ self.options.animation +'-show');
					...
					// if the tooltip isn't already open, open that sucker up!
						...
						var animation = 'tooltip-' + self.options.animation,
						...
						self.$tooltip = $('<div class="tooltip-base '+ self.options.theme +'" style="'+ minWidth +' '+ maxWidth +' '+ pointerEvents +' '+ animationSpeed +'"><div class="tooltip-content"></div></div>');
		...
		_content_insert: function() {
			...
			var self = this,
				$d = this.$tooltip.find('.tooltip-content');
				...
				if (self.Status !== 'hidden') {
					...
					if (self.options.updateAnimation) {
						...
						if (GRID.browserSupportsTransitions()) {
							...
							}).addClass('tooltip-content-changing');
							...
							setTimeout(function() {
								...
								if(self.Status != 'hidden'){
									...
									self.$tooltip.removeClass('tooltip-content-changing');
		...
		hide: function(callback) {
			...
			if (self.Status == 'shown' || self.Status == 'appearing') {
				...
				if (GRID.browserSupportsTransitions()) {
					...
					self.$tooltip
						.clearQueue()
						.removeClass('tooltip-' + self.options.animation + '-show')
		...
		reposition: function() {
			...
			if ($('body').find(self.$tooltip).length !== 0) {
				...
				if (self.options.arrow) {
					...
					var arrowClass = 'tooltip-arrow-' + practicalPosition;
					...
					if (tooltipBorderWidth !== 0) {
						...
						arrowBorder = '<span class="tooltip-arrow-border" style="'+ arrowBorderSize +' '+ arrowBorderColor +';"></span>';
					}

					// if the arrow already exists, remove and replace it
					self.$tooltip.find('.tooltip-arrow').remove();

					// build out the arrow and append it
					var arrowConstruct = '<div class="'+ arrowClass +' tooltip-arrow" style="'+ arrowReposition +'">'+ arrowBorder +'<span style="border-color:'+ arrowColor +';"></span></div>';
		...
		destroy: function() {
			...
			if(ns.length === 1){
				...
				if (self.options.restoration === 'previous'){
					title = self.$el.data('tooltip-initialTitle');
				}
				...
				self.$el
					...
					.removeData('tooltip-initialTitle');
			}
			else {
